...
- **[이미지영어] 커리큘럼 전면 도입**: 기존 초급영어를 대체하여, 원어민의 물리적/시각적 마인드셋(공간, 방향 이동, 수직 압력, 분리 등)을 다루는 20개 레슨과 맞춤형 다이어그램 일러스트레이션을 탑재했습니다. (이미지 여백 Crop 최적화 완비)
- **퍼블릭 학습 UI/UX 대폭 개편**: 좌측 목차 리스트(Sidebar, 480px)와 우측 본문(Content) 영역이 화면 내에서 **독립적으로 스크롤(Independent Scrolling)** 되도록 구조를 혁신했습니다. 전체 레이아웃을 좌측 정렬하고, 불필요한 시각적 여백을 압축(Compact)하여 가독성을 높였습니다.
- **관리자 로그인 편의성 (보안 강화)**: 관리자 로그인 폼에 쿠키를 활용한 **'아이디 저장(Remember ID)'** 및 세션 만료기간(JWT)을 30일로 연장하는 **'자동 로그인(Auto Login)'** 기능을 추가했습니다.
- **2-Level 카테고리 (Depth Navigation) 지원**: 단순 1단계 강좌 나열을 넘어, 상단 GNB 바에서 드롭다운 메뉴를 통해 대분류(Category) 안의 개별 강좌(Course)들로 직관적인 탐색이 가능해졌습니다.
- **신규 전문 매뉴얼 구축**: 기본 EPC 매뉴얼 외에, 현장 실무자를 위한 **[철골 공사 매뉴얼]** 및 **[철근 공사 매뉴얼]**과 그에 따르는 고화질 전문 용어집(Glossary) 데이터를 시딩 스크립트(`scripts/`)로 완비했습니다.
- **[건설법규] 심화 강좌 및 실무 계약서 양식 탑재**: 전문건설사 관점에서의 하도급법, 건산법, 파견법 등 법무 지식과 실무 대처 방안을 담은 대규모 강좌 데이터(`seed_law_ultimate_pt1.py ~ pt3.py`)를 추가했습니다.
- **강구조물 불법 재하도급 방지용 핵심 계약서 10종 지원**: 물품공급, 장비임대, NDT 위탁, 자문, 함바식당, 일용직 근로계약서 등 실무에서 즉시 활용 가능하고 법적 방어력을 갖춘 계약서 템플릿(`seed_law_ultimate_pt4.py`)을 강좌 내에 새롭게 배포했습니다.
- **가독성 향상 및 스크립트 자원 최적화**: 어두운 배경(Dark mode 요소)을 밝은 톤으로 전면 교체하여 강의 본문 가독성을 크게 높였으며, 불필요한 테스트용 시드 스크립트들을 일제 정리하여 프로젝트 구조를 다이어트했습니다.
