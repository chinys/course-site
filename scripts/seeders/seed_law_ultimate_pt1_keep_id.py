...
def seed_law_course_ultimate():
    ...
    if row:
        ...
        cursor.execute('''
            UPDATE course
            SET description = '건설산업기본법 (건산법) 을 완벽 해부하는 14 강 체제 궁극의 실무 매뉴얼. 하도급법, 도급계약서, 중대재해처벌법 가이드까지 현장 관리자 및 실무진을 위한 필수 지침서입니다.',
                thumbnail_url = ?, category_id = ?
            WHERE id = ?
        ''', (thumbnail_path, category_id, course_id))
        print(f"[UPDATE] 강좌 '{course_title}' 업데이트 (ID: {course_id})")
    else:
        cursor.execute('''
            INSERT INTO course (title, description, thumbnail_url, category_id)
            VALUES (?, ?, ?, ?)
        ''', (course_title, "건설산업기본법 (건산법) 을 완벽 해부하는 14 강 체제 궁극의 실무 매뉴얼. 하도급법, 도급계약서, 중대재해처벌법 가이드까지 현장 관리자 및 실무진을 위한 필수 지침서입니다.", thumbnail_path, category_id))
        course_id = cursor.lastrowid
        print(f"[CREATE] 강좌 '{course_title}' 생성 (ID: {course_id})")

    # 3. 레슨 데이터 (1-5 강)
    lessons = [
        (1, '1. [건산법 총칙] 건설산업기본법의 목적과 적용 범위', get_lesson_1_content()),
        (2, '2. [건산법 등록] 건설업의 등록체계와 결격사유', get_lesson_2_content()),
        (3, '3. [건산법 도급/하도급-1] 도급의 원칙과 다단계 하도급 금지', get_lesson_3_content()),
        (4, '4. [건산법 도급/하도급-2] 불공정 행위 금지 및 대금 직불 보장', get_lesson_4_content()),
        (5, '5. [건산법 현장관리] 현장대리인 지정, 전자카드제 및 표지 실명제', get_lesson_5_content()),
    ]

    # 4. 레슨 업데이트 또는 생성 (DELETE 안 함!)
    for order, title, content in lessons:
        # 기존 레슨 확인 (course_id + order 로)
        cursor.execute(
        ...
        if row:
            ...
            print(f"  [CREATE] Lesson {new_id}: {title[:30]}...")

    conn.commit()
    conn.close()
    print("\n[OK] 건설법규 1-5 강 업데이트 완료!")
...
def get_lesson_1_content():
    return '''<h2 class="text-3xl font-extrabold mb-6 text-gray-900 border-b-2 border-slate-200 pb-3">건설산업기본법 (건산법) 의 개요</h2>
    ...
    <h3 class="text-xl font-bold text-gray-900 border-l-4 border-blue-600 pl-3 mb-4">1.1. 주요 용어의 정의 (제 2 조)</h3>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b hover:bg-gray-50 transition text-gray-900">
                    ...
                    <td class="p-4 text-gray-900 leading-relaxed"><ul class="list-disc pl-5 space-y-1"><li><strong>도급:</strong> 발주자와 수급인 간, 또는 수급인과 하수급인 간에 일의 완성을 약정하고 이에 대한 대가를 지급할 것을 약정하는 계약.</li><li><strong>하도급:</strong> 도급받은 건설공사의 전부 또는 일부를 다시 위탁하기 위하여 수급인이 제 3 자와 체결하는 도급계약.</li></ul></td>
                </tr>
    ...
    <h3 class="text-xl font-bold text-gray-900 border-l-4 border-blue-600 pl-3 mb-4">1.2. 타 법률과의 관계 (제 3 조)</h3>
...
def get_lesson_2_content():
    return '''<h2 class="text-3xl font-extrabold mb-6 text-gray-900 border-b-2 border-slate-200 pb-3">건설업 면허 체계의 법적 이해</h2>
    ...
    <h3 class="text-xl font-bold text-gray-900 border-b-2 border-gray-200 pb-2 mb-4">2.1. 건설업의 종류 및 등록 요건 (제 9 조)</h3>
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-8">
        <div class="border p-5 rounded-xl bg-white shadow-md border-blue-200 transition">
            <h4 class="font-bold text-indigo-900 mb-4 text-lg border-b border-indigo-200 pb-2 flex items-center gap-2"><span>🏗️</span> 종합건설업 (5 개 업종)</h4>
            <ul class="text-sm text-gray-900 list-none space-y-2">
                <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-indigo-200"></span>건축공사업 (자본금 3.5 억원 이상, 관련 기술인 5 인 이상)</li>
                <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-indigo-200"></span>토목공사업 (자본금 5 억원 이상, 관련 기술인 6 인 이상)</li>
                <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-indigo-200"></span>토목건축공사업 (자본금 8.5 억원 이상, 관련 기술인 11 인 이상)</li>
        ...
        <div class="border p-5 rounded-xl bg-white shadow-md border-teal-200 transition">
            ...
            <p class="text-xs text-gray-900 mb-3 bg-teal-50 p-2 rounded shadow-inner">2022 년 시행령 개정으로 전문건설업 업종이 대업종 단위로 통합 운영되고 있습니다.</p>
            <ul class="text-sm text-gray-900 list-none space-y-2">
                ...
                <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal-200"></span>철근·콘크리트공사업 등 29 개 업종</li>
            </ul>
        </div>
    </div>
    ...
    <div class="bg-red-50 p-6 rounded-xl border border-red-200 mb-8 shadow-sm">
        ...
        <div class="bg-white p-4 rounded-lg border border-red-100 mb-3 shadow-inner">
            <ul class="text-sm font-semibold text-gray-900 list-disc pl-5 space-y-2">
                <li><strong>종합공사:</strong> 예정금액 <strong>5 천만원 미만</strong> (부가가치세 포함)</li>
                <li><strong>전문공사:</strong> 예정금액 <strong>1 천 5 백만원 미만</strong> (부가가치세 포함)</li>
            </ul>
        </div>
    ...
    <h3 class="text-xl font-bold text-red-900 border-b-2 border-gray-200 pb-2 mb-4">2.3. 무면허 시공 및 명의대여 금지 규정 (제 21 조)</h3>
    ...
    <div class="bg-gray-50 text-gray-900 font-bold p-5 rounded-xl shadow-md border border-gray-300">
        <p class="text-sm text-gray-900 leading-relaxed font-semibold"><strong class="text-gray-900 mr-2">강행 규정 및 벌칙 (제 96 조):</strong> 무등록 시공자, 명의 대여자 및 차용자, 관련 알선자 전원은 <strong class="text-red-900 text-lg mx-1">5 년 이하의 징역 또는 5 천만원 이하의 벌금</strong>에 처해집니다.<br>더불어, 해당 위반 사업자는 행정 처분으로 <strong>필요적 등록 말소 (면허 영구 취소)</strong> 대상이 됩니다.</p>
    </div>'''


def get_lesson_3_content():
    return '''<h2 class="text-3xl font-extrabold mb-6 text-gray-900 border-b-2 border-slate-200 pb-3">건설공사 도급계약 및 하도급 규제</h2>
    ...
    <div class="grid grid-cols-1 mb-8 gap-5">
        <div class="border-l-4 border-gray-600 bg-white p-5 rounded-r-xl shadow-sm transition">
            <h3 class="text-xl font-bold text-gray-900 mb-3">🚫 일괄 하도급의 원칙적 금지 (제 29 조 제 1 항)</h3>
        ...
        <div class="border-l-4 border-gray-600 bg-white p-5 rounded-r-xl shadow-sm transition">
            <h3 class="text-xl font-bold text-gray-900 mb-3">🔄 동일 업종 간 하도급의 제한 (제 29 조 제 2 항)</h3>
        ...
        <div class="border-l-4 border-gray-600 bg-gray-50 p-5 rounded-r-xl shadow-md">
            <h3 class="text-xl font-bold text-gray-900 mb-3">⚠️ 재하도급 행위의 원칙적 금지 (제 29 조 제 3 항)</h3>
            <p class="text-gray-900 mb-3 font-semibold text-lg">"하수급인은 자신이 도급받은 건설공사를 제 3 자에게 다시 위탁 (재하도급) 할 수 없다."</p>
    ...
    <h3 class="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">직접 시공 의무 (제 28 조의 2)</h3>
    ...
    <div class="overflow-hidden bg-white rounded-xl border border-gray-200 shadow-sm text-sm mb-4">
        <table class="w-full text-center border-collapse">
            ...
            <tbody class="divide-y divide-gray-200 text-gray-900">
                <tr class="hover:bg-gray-50"><td class="p-4">3 억원 미만</td><td class="p-4 font-bold text-gray-900">50% 이상</td></tr>
                <tr class="hover:bg-gray-50"><td class="p-4">3 억원 이상 ~ 10 억원 미만</td><td class="p-4 font-bold text-gray-900">30% 이상</td></tr>
                <tr class="hover:bg-gray-50"><td class="p-4">10 억원 이상 ~ 30 억원 미만</td><td class="p-4 font-bold text-gray-900">20% 이상</td></tr>
                <tr class="hover:bg-gray-50"><td class="p-4">30 억원 이상 ~ 70 억원 미만</td><td class="p-4 font-bold text-gray-900">10% 이상</td></tr>
                <tr class="bg-gray-50"><td class="p-4">70 억원 이상</td><td class="p-4">적용 제외 (자율 시공)</td></tr>
            </tbody>
        </table>
    </div>'''
...
def get_lesson_4_content():
    return '''<h2 class="text-3xl font-extrabold mb-6 text-gray-900 border-b-2 border-slate-200 pb-3">공정거래 질서 확립과 하도급대금 지급 보증체계</h2>
    ...
    <h3 class="text-xl font-bold text-gray-900 bg-gray-100 py-2 px-4 rounded-t-lg border-x border-t border-gray-300">1. 부당한 하도급대금 산정 금지 (제 38 조)</h3>
    <div class="border-x border-b border-gray-300 p-5 rounded-b-lg mb-8 bg-white shadow-sm">
        ...
        <div class="bg-gray-50 p-4 border border-gray-200 rounded text-sm text-gray-900">
            <strong class="text-gray-900 flex items-center gap-1 mb-2"><span>🔍</span> 발주자의 하도급 적정성 심사 의무 (제 82 조):</strong>
    ...
    <h3 class="text-xl font-bold text-gray-900 bg-gray-100 py-2 px-4 rounded-t-lg border-x border-t border-gray-300 mt-6 flex justify-between items-center">
        <span>2. 하도급대금 직접 지급 사유 (제 35 조)</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="bg-gray-50 border border-gray-200 p-5 rounded-xl">
                ...
                <ul class="list-disc pl-5 space-y-2 text-sm text-gray-900 font-medium">
                    <li>발주자, 수급인, 하수급인 3 자 간 하도급대금 직접 지급에 관한 '사전 합의서'를 작성한 경우.</li>
                    <li>수급인이 <strong>2 회 이상의 하도급대금 지급 지체 사실</strong>이 발생한 경우.</li>
            ...
            <div class="bg-gray-50 border border-gray-200 p-5 rounded-xl">
                <h4 class="font-bold text-base text-gray-900 mb-3 border-b border-gray-200 pb-2">건설기계 대여대금 지급보증 의무 (제 68 조의 3)</h4>
    ...
    <h3 class="text-xl font-bold text-gray-900 bg-gray-100 py-2 px-4 rounded-t-lg border-x border-t border-gray-300 mt-6 flex justify-between items-center">
        <span>3. 노무비 (임금) 에 대한 압류의 절대적 금지 (제 88 조)</span>
        <span class="text-xs bg-gray-200 text-gray-900 px-2 py-1 rounded font-bold">노임 권익 보호</span>
    </h3>
    <div class="border-x border-b border-gray-300 p-6 rounded-b-lg mb-8 bg-white shadow-sm">
        <p class="text-gray-900 mb-4 leading-relaxed">건설사업자의 재정 악화로 제 3 의 채권자가 채권을 가압류하려는 절차를 개시하더라도, 도급금액 구성 성분 중 <strong>"근로자의 임금에 명백히 해당하는 노임 상당액"</strong>에 대해서는 제 3 자가 강제집행이나 압류를 진위할 수 없습니다.</p>
...
def get_lesson_5_content():
    return '''<h2 class="text-3xl font-extrabold mb-6 text-gray-900 border-b-2 border-slate-200 pb-3">안전한 건설공사 수행을 위한 체계적 현장 통제</h2>
    ...
    <h3 class="text-xl font-bold text-gray-900 border-l-4 border-gray-600 pl-3 mb-4">1. 건설기술인 (현장대리인) 의 배치 의무 (제 40 조)</h3>
    <p class="text-gray-900 mb-4 text-sm leading-relaxed">건설사업자는 당해 공사의 특성 및 규모에 부합하는 적격 <strong>분야 및 기술 등급 (초급~특급 등)</strong> 을 소지한 건설기술인을 1 명 이상 현장마다 의무적으로 현장대리인으로 배치하여야 합니다.</p>
    ...
    <div class="bg-gray-50 border border-gray-300 rounded-xl p-6 mb-8 shadow-sm">
        ...
        <ul class="list-disc pl-5 space-y-3 text-sm text-gray-900">
            ...
            <li><strong class="text-gray-900">단일 관장 원칙:</strong> 안전 및 품질 감독 권한의 약화 방지를 위해 원칙적으로 1 인의 건설기술인은 하나의 현장에 전속적으로 소속되어 통제 업무를 수행해야 합니다.</li>
            <li><strong class="text-gray-900">예외 조항 (영세 현장의 예외):</strong> 예정금액이 일정 규모 (예: 5 억 원) 미만인 동일 권역 내 현장에 한정하여, 발주자 전원의 승낙을 득한 경우 최대 3 곳까지 중복하여 현장 관리 업무를 포괄 배분할 수 있습니다. (허위 기재 방지 필요)</li>
        </ul>
    </div>

    <h3 class="text-xl font-bold text-gray-900 border-l-4 border-gray-600 pl-3 mb-4">2. 투명한 노무관리를 위한 건설근로자 전자카드제 전면화</h3>
    <p class="text-gray-900 mb-3 text-sm leading-relaxed">하도급 구조상 빈번히 제기되어 온 노무 인력 정보의 오기 및 미보고를 예방하고 투명한 퇴직보증 환경을 조성하고자 제정된 절차입니다. 2024 년 이후 민간을 포함한 대부분의 건설 현장으로 확대 적용 중입니다.</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="border border-gray-200 bg-gray-50 p-5 rounded-lg shadow-sm">
            ...
            <p class="text-sm text-gray-900 leading-relaxed">발주자의 감독권 보장 및 투명화 정책의 일환으로, 계약 당사자는 도급 약정 성립 혹은 하도급 업체 지정 후 관련 세부 정보를 <strong>기한 (체결일로부터 30 일 이내) 내에 KISCON(건설산업지식정보시스템) 에 등재하고 이를 통보</strong>하여 현황 집계를 가능케 해야 합니다.</p>
        </div>
    </div>'''
